...
function showFirstAndLast(arr){
    ...
    arr.forEach(function(val) {
        const str = val[0] + val[val.length - 1];
        newarr.push(str);
    });
    return newarr;
...
function vowelCount(str){
    ...
    splitstr.forEach(el => {
        ...
        if (vowels.includes(lowerel))
        {
            if(vowelobj[lowerel] != null)
            {
                vowelobj[lowerel] = vowelobj[lowerel] + 1;
            }
            else
            {
                vowelobj[lowerel] = 1;
